...
//*******************
	...
	//*モードクリア表示*
		...
		ModeClearLabel.update = function(){
			...
			if(rank==0){
				this.text = "<div id='clear_label'>未クリア<br>ハイスコア:"+high_score+"点</div>";
			}else if(rank==1){
				this.text = "<div id='clear_label'>★<br>ハイスコア:"+high_score+"点</div>";
			}else if(rank==2){
				this.text = "<div id='clear_label'>★★<br>ハイスコア:"+high_score+"点</div>";
			}else if(rank==3){
				this.text = "<div id='clear_label'>★★★<br>ハイスコア:"+high_score+"点</div>";
		...
		LastBall.decrement = function(){
			...
			if(this.num == 0 && this.battedball_num == 0 && Pitcher.throw_flag == true && SceneBatting.bgm_fadeout == false){
				...
				setTimeout(function(){
					...
					//------クッキー処理-----
					cookieUpdate(GameSet[Mode][Course]["course_e"], Point, GameSet[Mode][Course]["ball_number"]);
					//--------------------
